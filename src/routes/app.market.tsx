...
import { TrendingUp, TrendingDown, Gauge, ArrowUpRight, ArrowDownRight, Calendar, Activity } from "lucide-react";
...
function IndexCard({ name, value, chg, pct, up, seed }: any) {
  return (
    <div className="rounded-xl border border-white/10 bg-card/60 p-4">
      ...
      <p className={`text-xs ${up ? "text-emerald-400" : "text-red-400"}`}>{chg} ({pct})</p>
      <div className="mt-2"><MiniChart trend={up ? "up" : "down"} seed={seed} /></div>
    </div>
  );
}
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <IndexCard name="NIFTY 50" value="24,812.40" chg="+184.30" pct="+0.75%" up seed={1} />
        <IndexCard name="BANK NIFTY" value="51,420.05" chg="-126.40" pct="-0.25%" up={false} seed={2} />
